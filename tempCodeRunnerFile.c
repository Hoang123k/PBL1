...
void sapxeptheoGiasp(Hanghoa l){
    if (empty(l)) return;
    for (Hanghoa i = l; i->next != NULL; i = i->next) {
        for (Hanghoa j = i->next; j != NULL; j = j->next) {
            if (i->gia > j->gia) { // Sắp xếp tăng dần
                // Chỉ hoán đổi dữ liệu, giữ nguyên liên kết next
                struct SanPham temp = *i;
                
                // Copy data (tránh đè lên con trỏ next)
                strcpy(i->masp, j->masp);
                strcpy(i->tensp, j->tensp);
                strcpy(i->size, j->size);
                i->gia = j->gia;
                i->soluong = j->soluong;

                strcpy(j->masp, temp.masp);
                strcpy(j->tensp, temp.tensp);
                strcpy(j->size, temp.size);
                j->gia = temp.gia;
                j->soluong = temp.soluong;
            }
        }
    }
}
void sx(Hanghoa *l) { 
    ...
    while(test) {
        ...
        switch(k) {
            ...
            case 4:
                sapxeptheoSLsp(*l);
                printf("Da sap xep theo so luong san pham!\n");
                stop_display();
                break;
            case 0:
                break;
